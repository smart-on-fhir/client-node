...
/**
 * The default storage factory assumes that `hapi-server-session` is used (or
...
module.exports = function(options) {
    ...
    return {
         ...
         * @param {Object} h
         */
        async authorize(req, h) {
            let { launch, iss, fhirServiceUrl } = req.query;
            if ((launch && iss) || fhirServiceUrl || cfg.serverUrl) {
                const url = await smart.buildAuthorizeUrl(req.raw.req, cfg, cfg.getStorage(req));
         ...
         * @param {Object} h
         */
        async refreshAuth(request, res, next) {
