...
export default class Client
{
     ...
     */
    async request(options: AxiosRequestConfig | string): Promise<any>
    {
        ...
        let accessToken = getPath(this.state, "tokenResponse.access_token");
        if (accessToken) {            
            options.headers = {
                ...options.headers,
            ...
            .catch(error => {
                ...
                if (error.response) {
                    ...
                    if (body &&
                        ...
                        let errors = body.issue.map((o: any) => `${o.severity} ${o.code} ${o.diagnostics || ""}`);
    ...
    refresh()
    {
        ...
        let refreshToken = getPath(this.state, "tokenResponse.refresh_token")
